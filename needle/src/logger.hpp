...
namespace logger
{
    ...
    {
      return static_cast<logger::option>
      (static_cast<std::underlying_type_t<logger::option>>(x) & static_cast<std::underlying_type_t<logger::option>>(y));
    }
    ...
    {
      detail::ensure_init();
      logger::set_error_option(option::BG_DARK_RED | option::FG_BLACK);
